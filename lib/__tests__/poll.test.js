...
import { runPoll, pollInterval, discoveryInterval } from '../poll.js';
...
describe('runPoll', () => {
  ...
  test('discoveryInterval: daily when nothing is happening, half-hourly around a live event', () => {
    const now = T0;
    assert.equal(discoveryInterval({ now }), 24 * HOUR);
    assert.equal(discoveryInterval({ lastChangeAt: now - 3 * HOUR, now }), 30 * 60_000);
    assert.equal(discoveryInterval({ lastChangeAt: now - 30 * HOUR, now }), 24 * HOUR);
    assert.equal(discoveryInterval({ failed: true, now }), 2 * HOUR);
  });

  test('a discovered event carries its start number, so no starting-rank download', async () => {
    const ctx = await setup({
      follows: [{ id: 'me', playerName: 'Suriyajan, Chayapol', fideId: '6200456', isMe: true, autoDiscover: true, tournaments: [] }],
    });
    ctx.site.fetchers.discoverTournaments = async () => [
      { id: '555', label: 'Found Open', endDate: '2026-09-21', startNo: 7, name: 'Suriyajan, Chayapol' },
    ];
    await tick(ctx, T0);
    assert.deepEqual(ctx.site.calls, ['card:555:7'], 'one request, not three');

    // ...and the search does not run again on the next tick.
    ctx.site.calls.length = 0;
    await tick(ctx, T0 + 60_000);
    assert.deepEqual(ctx.site.calls, ['card:555:7']);
  });

  test('the search runs again a day later, or straight away once its record is cleared', async () => {
    const ctx = await setup({
      follows: [{ id: 'me', playerName: 'Suriyajan, Chayapol', isMe: true, autoDiscover: true, tournaments: [] }],
    });
    await tick(ctx, T0);
    ctx.site.calls.length = 0;
    await tick(ctx, T0 + 6 * HOUR);
    assert.deepEqual(ctx.site.calls, [], 'no live event: daily is enough');

    await tick(ctx, T0 + 25 * HOUR);
    assert.deepEqual(ctx.site.calls, ['discover']);

    // "Find my events now" deletes the record.
    const poll = await ctx.store.get('poll');
    delete poll.discovery.me;
    await ctx.store.set('poll', poll);
    await tick(ctx, T0 + 25 * HOUR + 60_000);
    assert.deepEqual(ctx.site.calls, ['discover', 'discover']);
  });
