...
describe('notification text', () => {
  test('reads like a pairing card', () => {
    ...
    assert.equal(notification.title, 'RD 5 · BD 12');
    assert.equal(notification.body, 'vs Novák, Šimon (2145) · WHITE');
    assert.equal(notification.url, '/t/999');
  });
  ...
  test('degrades gracefully when board, rating or colour are missing', () => {
    ...
    assert.equal(notification.title, 'RD 1');
    assert.equal(notification.body, 'vs Someone');
  });

  test('names the player when it is someone you follow', () => {
    const notification = buildNotification({
      tournamentId: '999', startNo: 7, round: 3, board: 4, opponent: 'X', isMe: false, playerName: 'Kantor, Sam',
    });
    assert.equal(notification.title, 'Kantor · RD 3 · BD 4');
    assert.equal(notification.url, '/t/999?p=7');
  });
