...
import { parseTournamentTitle } from './tournament.js';
...
/**
 * Fetch and parse one player's card in a tournament. The page's heading names the
 * tournament, so the title comes back for free — the poller no longer needs the
 * starting-rank page just to learn what the event is called.
 */
export async function fetchPlayerCard(tournamentId, startNo) {
  ...
  return { url, title: parseTournamentTitle(html), rounds: parsePlayerCard(html) };
}
