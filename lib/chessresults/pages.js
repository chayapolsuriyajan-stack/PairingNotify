...
/** One player's card: rounds plus the header facts (FIDE ID, performance, rank...). */
export async function fetchPlayer(id, startNo) {
  const html = await fetchHtml(tournamentUrl(id, { art: '9', snr: String(startNo) }));
  return { rounds: parsePlayerCard(html), header: parsePlayerHeader(html) };
}
