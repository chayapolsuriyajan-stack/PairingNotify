 ...
 */
export async function fetchHtml(path, options) {
  return (await fetchPage(path, options)).html;
}

/**
 * Like fetchHtml, but also returns the final URL after redirects. chess-results
 * redirects to a mirror (s1/s2/s3.chess-results.com), and a WebForms POST must go back
 * to the same mirror that issued its __VIEWSTATE.
 *
 * @returns {Promise<{html: string, url: string}>}
 */
export async function fetchPage(path, { form, retries = 3 } = {}) {
